...
#include "Unknow_MCP3008.h"
#include <SPI.h>

bool Unknow_MCP3008::begin(uint8_t cs, SPIClass* theSPI) {
  hwSPI = true;
...
bool Unknow_MCP3008::begin(uint8_t sck, uint8_t mosi, uint8_t miso,
                             uint8_t cs) {
  hwSPI = false;
...
int Unknow_MCP3008::readADC(uint8_t channel) {
  if (channel > 7)
    return -1;
  return SPIxADC(channel, false);
}

int Unknow_MCP3008::readADCDifference(uint8_t differential) {
  if (differential > 7)
    return -1;
...
int Unknow_MCP3008::SPIxADC(uint8_t channel, bool differential) {
